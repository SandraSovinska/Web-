function displayStudents(students, element) {
    ...
    for (let index = 0; index < students.length; index++) {
        ...
        let template = ' <div class="col m4 s12"> <div class="card"> <div class="card-image waves-effect waves-block waves-light"> <img class="activator" src="{{photo}}"> </div><div class="card-content"> <span class="card-title activator grey-text text-darken-4">{{name}} {{surname}}<i class="material-icons right">more_vert</i></span> <p>Dokumentas: {{documentId}}</p></div><div class="card-reveal"> <span class="card-title grey-text text-darken-4">{{name}} {{surname}}<i class="material-icons right">close</i></span> <p>Here is some more information about this product that is only revealed once clicked on.</p></div></div></div>';
